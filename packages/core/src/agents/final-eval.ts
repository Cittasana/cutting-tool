...
export async function evaluateFinal(opts: {
  apiKey: string;
  reelFrameBase64Pngs: string[];
  brief: MarketingBrief;
  storyboard: Storyboard;
  veoVoiceoverTranscript: string;
}) {
  const { apiKey, reelFrameBase64Pngs, brief, storyboard, veoVoiceoverTranscript } = opts;
  const imageBlocks = await imageBlocksFromBase64Pngs(reelFrameBase64Pngs);
...
${reelFrameBase64Pngs.length} sample frames from the assembled reel follow.
